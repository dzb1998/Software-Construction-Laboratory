...
// Fill the thread array, move the code from main to here
void *fillThreadArr(void *thread_num) {
    ...
    // declare a thread number; cast to int* then dereference
    int th_num = *(int*) thread_num;
    
    /* for every pixel */
    for( int px=th_num; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            // Then put them into thread array
            //for (int i = 0; i < 3; i++) {
            //    threadArr[px][py][i] = scaled_color[i];
            //}
...
//until here
    ...
    // Now support the Multithreading
    pthread_t tid[nthreads];
    int thread_num[nthreads];
    int ret;
    for (int n = 0; n < nthreads; n++) {
        thread_num[n] = n;
        //ret = pthread_create(&tids[t], NULL, printMsg, (void *) t); given in ppt
        ret = pthread_create(&tid[n], NULL, fillThreadArr, &thread_num[n]);
        if (ret) {
            fprintf(stderr, "Error! Can't create the thread!");
            return 1;
        }
    }
    
    // After all the thread created
    for (int n = 0; n < nthreads; n++) {
    ...
    // Finally print the thread array
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
             ...
             */
            printf("%.0f %.0f %.0f\n",
                   threadArr[i][j][0], threadArr[i][j][1], threadArr[i][j][2]);
        }
        printf("\n");
    }
